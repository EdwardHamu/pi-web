...
test("prompt commands apply the requested model before the SDK snapshots context", async (t) => {
  const previousModel = { provider: "previous", id: "previous-model" };
  const requestedModel = { provider: "requested", id: "requested-model" };
  const inner = makePromptInner((_message, options) => {
    assert.deepEqual(inner.model, requestedModel);
    options.preflightResult(true);
    return Promise.resolve();
  });
  inner.model = previousModel;
  inner.modelRuntime = {
    getModel: (provider, modelId) => provider === requestedModel.provider && modelId === requestedModel.id
      ? requestedModel
      : undefined,
    refresh: async () => {},
  };
