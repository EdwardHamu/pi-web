...
import { loadPinnedSessionIds, savePinnedSessionIds } from "@/lib/pinned-sessions";
...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone, onSessionsChange }: Props) {
  ...
  // Pinning is a browser-local sidebar preference. Load it after hydration so
  // the server render and the first client render remain identical.
  useEffect(() => {
    setPinnedSessionIds(loadPinnedSessionIds());
    setPinnedSessionsHydrated(true);
  }, []);

  useEffect(() => {
    if (!pinnedSessionsHydrated) return;
    savePinnedSessionIds(pinnedSessionIds);
  }, [pinnedSessionIds, pinnedSessionsHydrated]);

  // Session files can be removed outside this sidebar. Do not leave their
  // ids in localStorage once a successful session refresh proves they are
  // gone.
  useEffect(() => {
    if (!pinnedSessionsHydrated || loading || error) return;
    const existingIds = new Set(allSessions.map((session) => session.id));
    setPinnedSessionIds((prev) => {
      const next = new Set([...prev].filter((id) => existingIds.has(id)));
      return next.size === prev.size ? prev : next;
    });
  }, [allSessions, error, loading, pinnedSessionsHydrated]);
  ...
  const handleTogglePinned = useCallback((sessionId: string) => {
    setPinnedSessionIds((prev) => {
      const next = new Set(prev);
  ...
  const orderedSessionFamilies = [...sessionFamilies].sort((a, b) => {
    const aPinned = pinnedSessionIds.has(a.root.id);
    const bPinned = pinnedSessionIds.has(b.root.id);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flex: explorerOpen && (selectedCwdProp || selectedCwd) ? "1 1 0" : "1 1 auto", overflowY: "auto", padding: "0", minHeight: 80 }}>
        ...
        {orderedSessionFamilies.map((family) => {
          ...
          return (
            <SessionItem
              ...
              onTogglePinned={() => handleTogglePinned(family.root.id)}
...
function RunningSessionIndicator() {
  ...
  return (
    <span
      className="pi-web-animation-layer"
    ...
    >
      <svg className="running-session-spinner" width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true" style={{ display: "block" }}>
        <g>
          <path
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, flexShrink: 0, marginLeft: 6 }}>
      {activity.running > 0 && (
        ...
        >
          <svg className="running-session-spinner" width="10" height="10" viewBox="0 0 24 24" fill="none" aria-hidden="true" style={{ display: "block" }}>
            <g>
          ...
          {hovered && !session.transient && (
            <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
              <button
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = isPinned ? "var(--bg-selected)" : "var(--bg-hover)";
