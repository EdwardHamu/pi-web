...
test("new-session implicit model previews do not overwrite the saved default", async () => {
  ...
  assert.match(routeSource, /persistModelDefault === false/);
  assert.match(managerSource, /persistModelDefault !== false && effectiveInitialModel/);
});
...
test("prompt admission synchronizes the client-selected model before calling the SDK", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const promptSource = source.slice(
    source.indexOf('case "prompt"'),
    source.indexOf('case "abort"'),
  ...
  assert.match(promptSource, /await this\.applyRequestedModel\(command\)/);
  assert.ok(promptSource.indexOf("await this.applyRequestedModel(command)") < promptSource.indexOf("this.inner.prompt"));
  assert.match(setModelSource, /await this\.acquirePromptAdmission\(\)/);
  assert.match(source, /private async applyRequestedModel\(command: Record<string, unknown>\)/);
});
